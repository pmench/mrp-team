...
# state_support <- df %>% 
#   group_by(state) %>%
#   summarise(raw_estimate = mean(as.integer(vote_choice_recoded) - 1, na.rm = TRUE), 
#             raw_estimate_sd = sd(as.integer(vote_choice_recoded) - 1), 
#             n = n(),
#             raw_estimate_se = raw_estimate_sd / sqrt(n)) %>% 
#   ggplot() + 
#   geom_point(aes(x = state, y = raw_estimate), color = '#e32245') + 
#   geom_errorbar(aes(ymin = raw_estimate - 2 * raw_estimate_se, ymax = raw_estimate + 2 * raw_estimate_se, x = state), 
#                 alpha = 0.5, width = 0, color = '#e32245') + 
...
raw_ests_plot <- ests %>%
  group_by(source) %>%
  summarise(estimate = median(estimate), estimate_se = median(estimate_se)) %>%
  ggplot() + 
  geom_point(aes(x = 0.5, y = estimate, color = source)) + 
  geom_errorbar(aes(ymin = estimate - 2 * estimate_se, ymax = estimate + 2 * estimate_se, x = 0.5, color = source), 
                alpha = 0.5, width = 0) + 
  scale_y_continuous(breaks=c(0,.25,.5,.75,1), 
                     labels=c("0%","25%","50%","75%","100%"), 
                     expand=c(0,0))+
...
```{r}
bayesplot_grid(ests_plot,raw_ests_plot, 
               grid_args = list(nrow=1, widths=c(5, 2)))
```

```{r}
states_map <- us_map(regions = "states")
states_map <- states_map %>% mutate(state = tolower(full))
states_df_melted <- state_support %>% select(state, estimate)
states_map <- left_join(states_map, states_df_melted, by = "state") %>% drop_na()
```
