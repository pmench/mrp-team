...
```{r}
post_strat <- read.csv("../data/census/cleaned/post_stratification_data.csv")

post_strat <- post_strat %>% mutate(STATEFIP = as.factor(STATEFIP), 
                      region = as.factor(region), 
                      age_recoded = as.factor(age_recoded), 
                      race_recoded = as.factor(race_recoded), 
                      male = as.factor(male), 
...
```{r}
tot = sum(post_strat$HHWT)
post_strat <- post_strat %>% mutate(prop = HHWT/tot)
```


```{r}
m4 <- stan_glmer(vote_choice_recoded ~ (1|male) + (1|race_recoded) + (1|education_recoded) + (1|age_recoded) + (1|region),
                  data = df2, prior = normal(location = 0, scale = 0.5, autoscale = TRUE),
                  prior_intercept = normal(location = 0, scale = 0.5, autoscale = TRUE), seed = 853)
```
...
support_by_group <-
  m4 |>
  add_epred_draws(newdata = post_strat) |>
  rename(support_trump_predict = .epred) |>
  mutate(support_trump_predict = support_trump_predict * prop) |>
  ungroup() |> 
  summarise(support_trump_predict = sum(support_trump_predict),
            .by = c(STATEFIP, male, race_recoded, education_recoded, age_recoded, .draw)) |>
  summarise(
    mean = mean(support_trump_predict),
    lower = quantile(support_trump_predict, 0.025),
    upper = quantile(support_trump_predict, 0.975),
    .by = c(STATEFIP, male, race_recoded, education_recoded, age_recoded)
  )


head(support_by_group)
```


```{r}
write.csv(support_by_group, 'propensity_by_group.csv')
```
