...
\[
P(Y_i = j) = \frac{e^{\beta_{0j} + \beta_{1j}x_1 + \ldots + \beta_{pj}x_p}}{1 + \sum_{k=1}^{J-1}e^{\beta_{0k} + \beta_{1k}x_1 + \ldots + \beta_{pk}x_p}}
\]
...
You can also examine the probabilities for each consumer and averaging all will give you the market share.

```{r}
head(heat_model$probabilities)
```

```{r}
colMeans(heat_model$probabilities)
```

Compare this to the true market share.
